...
void report_realtime_status()
{
  ...
  // Report realtime feed speed
    ...
    if (sys.report_ovr_counter > 0) { sys.report_ovr_counter--; }
      ...
      uint8_t liquid_state = liquid_get_state();
      if (liquid_state) {
        printPgmString(PSTR("|Lq:"));
        if (liquid_state & LIQUID_STATE_1) { serial_write('1'); }
        if (liquid_state & LIQUID_STATE_2) { serial_write('2'); }
        if (liquid_state & LIQUID_STATE_3) { serial_write('3'); }
        if (liquid_state & LIQUID_STATE_4) { serial_write('4'); }
      }
    }
  #endif
